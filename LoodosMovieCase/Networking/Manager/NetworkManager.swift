...
struct NetworkManager {
    ...
    func getMovieDetail(movieId: String, completion: @escaping (_ movieDetail: MovieDetailModel?,_ error: String?)->()){
                ...
                switch result {
                    case .success:
                        ...
                        do {
                            ...
                            let jsonData = try JSONSerialization.jsonObject(with: responseData, options: .mutableContainers)
                            print(jsonData)
                            let apiResponse = try JSONDecoder().decode(MovieDetailModel.self, from: responseData)
